...
Utility scripts for collecting (claiming) staking rewards, for voting on the (legacy) governance proposals, for moving REWA and DCDTs, and so on.
...
Supposing you have the [drt-sdk-testwallets](https://github.com/TerraDharitri/drt-sdk-testwallets) repository (the DharitrI test wallets) cloned into the `$HOME` directory.
...
[
    ...
    {
        "kind": "mnemonic",
        "mnemonicFile": "~/drt-sdk-testwallets/users/mnemonic.txt",
        "addressIndices": [4, 5]
    },
    {
        "kind": "keystore",
        "file": "~/drt-sdk-testwallets/users/alice.json",
        "password": "password",
        "addressIndices": []
    },
    {
        "kind": "keystore",
        "file": "~/drt-sdk-testwallets/users/bob.json",
        "passwordFile": "~/drt-sdk-testwallets/users/password.txt",
        "addressIndices": []
    },
    {
        "kind": "keystores",
        "folder": "~/drt-sdk-testwallets/users",
        "uniquePasswordFile": "~/drt-sdk-testwallets/users/password.txt"
    },
    {
        "name": "whale",
        "kind": "pem",
...
```
export RECEIVER="drt1testnlersh4z0wsv8kjx39me4rmnvjkwu8dsaea7ukdvvc9z396qe2mapq"
```
...
```
PYTHONPATH=. python3 ./wizard/prepare_custom_tokens.py --token=WREWA-a28c59 --network=devnet --wallets=$WALLETS_CONFIG --after-epoch=0 --threshold=0 --outfile=custom_transfers.json
...
Export auth registration entries (2FA secrets) to a **Mobile Authenticator App**: see [**drt-2fa-migration-tool**](https://github.com/TerraDharitri/drt-2fa-migration-tool).
