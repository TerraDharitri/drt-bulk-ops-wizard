...
from dharitri_py_sdk.abi import AddressValue, BigUIntValue, BytesValue, StringValue, U64Value
...
from wizard.guardians import AuthApp, AuthRegistrationEntry, CosignerClient, GuardianData
...
class MyEntrypoint:
    ...
    def recall_guardians(self, accounts: list[AccountWrapper]):
        ...
        def recall_guardian(wrapper: AccountWrapper):
            ...
            wrapper.guardian = (
                Address.new_from_bech32(guardian_data.active_guardian) if guardian_data.is_guarded else None
            )
    ...
    def get_claimed_rewards(self, delegator: Address, after_timestamp: int) -> list[ReceivedRewards]:
        ...
        for transaction in transactions:
            ...
            reward_result = next(
                (item for item in results if item.get("data") != CONTRACT_RESULTS_CODE_OK_ENCODED), None
            )
    ...
    def get_received_staking_rewards(self, node_owner: Address, after_timestamp: int) -> list[ReceivedRewards]:
        ...
        transactions = self._api_do_get(
            url, {"senderShard": METACHAIN_ID, "function": "reward", "after": after_timestamp, "size": size}
        )
    ...
    def vote_via_liquid_staking(
        self,
        sender: AccountWrapper,
        contract: str,
        proposal: int,
        vote: VoteType,
        power: int,
        ...
        transaction = controller.create_transaction_for_execute(
            ...
            arguments=[U64Value(proposal), StringValue(vote.value), BigUIntValue(power), BytesValue(proof)],
            gas_price=gas_price,
            guardian=sender.guardian,
        )
    ...
    def get_direct_vote(self, voter: Address, proposal: int) -> Optional[OnChainVote]:
        return self._get_past_vote(
            voter.to_bech32(), self.configuration.system_governance_contract, "vote", "vote", proposal
        )

    def get_vote_via_legacy_delegation(self, voter: Address, proposal: int) -> Optional[OnChainVote]:
        return self._get_past_vote(
            voter.to_bech32(), self.configuration.legacy_delegation_contract, "delegateVote", "delegateVote", proposal
        )
    ...
    def _get_past_vote(
        self, voter: str, contract: str, function: str, event_identifier: str, proposal: int
    ) -> Optional[OnChainVote]:
    ...
    def get_native_auth_access_tokens(self, account: IMyAccount) -> str:
        ...
        access_token = self.native_auth_client.get_token(
            address=account.address, token=init_token, signature=signature.hex()
        )
        return access_token
    ...
    def get_custom_tokens(self, address: Address, identifier_or_collection: str) -> list[Token]:
        ...
        # We have to perform this GET, so that we can observe all MetaDCDTs (all nonces) held by the account, as well.
        data_dcdt_and_meta: list[dict[str, Any]] = self.api_network_provider.do_get_generic(
        ...
        for item in data_dcdt_and_meta:
    ...
    def get_custom_token_balance_on_block_nonce(self, token: Token, address: Address, block_nonce: int) -> int:
        if token.nonce == 0:
            response = self.deep_history_proxy_network_provider.do_get_generic(
                f"address/{address.to_bech32()}/dcdt/{token.identifier}?blockNonce={block_nonce}"
            )
        else:
            response = self.deep_history_proxy_network_provider.do_get_generic(
                f"address/{address.to_bech32()}/nft/{token.identifier}/nonce/{token.nonce}?blockNonce={block_nonce}"
            )
    ...
    def send_multiple(
        self,
        auth_app: AuthApp,
        wrappers: list[TransactionWrapper],
        chunk_size: int = DEFAULT_CHUNK_SIZE_OF_SEND_TRANSACTIONS,
    ):
        ...
        for sender, transactions in grouped_by_sender.items():
            while True:
                try:
                    ...
                    print(
                        f"Unexpected error: [red]{error}[/red], will retry in {COSIGNER_SIGN_TRANSACTIONS_RETRY_DELAY_IN_SECONDS} seconds..."
                    )
    ...
    def await_processing_started(self, wrappers: list[TransactionWrapper]) -> list[TransactionOnNetwork]:
        ...
        transactions_on_network = Pool(NUM_PARALLEL_GET_TRANSACTION_REQUESTS).map(
            await_processing_started_one, wrappers
        )
    ...
    def await_completed(self, wrappers: list[TransactionWrapper]) -> list[TransactionOnNetwork]:
        def await_completed_one(wrapper: TransactionWrapper) -> TransactionOnNetwork:
            transaction_on_network = self.api_network_provider.await_transaction_completed(
                transaction_hash=wrapper.get_hash(), options=self.transaction_awaiting_options
            )
        ...
        transactions_on_network = Pool(NUM_PARALLEL_GET_TRANSACTION_REQUESTS).map(await_completed_one, wrappers)
