...
class AuthRegistrationEntry:
    def __init__(
        self, context: dict[str, Any], issuer: str, label: str, algorithm: str, digits: int, interval: int, secret: str
    ) -> None:
    ...
    def new_from_dictionary(cls, data: dict[str, Any]):
        ...
        label = data.get("account", "") or data.get("name", "") or data.get("label", "") or data.get("tag", "")
        algorithm = data.get("algorithm", "")
        digits = data.get("digits", 0)
        interval = data.get("interval", 0) or data.get("period", 0)
...
class CosignerClient:
    ...
    def sign_multiple_transactions(self, code: str, transactions: list[Transaction]):
        ...
        for index, transaction in enumerate(transactions):
            ...
            assert (transaction.guardian.to_bech32() if transaction.guardian else "") == signed_transactions[index].get(
                "guardian"
            )
...
class AuthApp:
    def __init__(self, registration_entries: list[AuthRegistrationEntry]) -> None:
        self.registration_entries_by_address: dict[str, AuthRegistrationEntry] = {
            entry.get_address(): entry for entry in registration_entries
        }
