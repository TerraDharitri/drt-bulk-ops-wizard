...
def _do_main(cli_args: list[str]):
    ...
    parser.add_argument(
        "--auth", required=True, help="auth registration entries (if exists, it will be updated in-place)"
    )
    ...
    for account_wrapper in accounts_wrappers:
        ...
        if registration_entry:
            print(
                f"... registration entry [blue]already available[/blue], guardian = {registration_entry.get_guardian()}"
            )
