...
def _do_main(cli_args: list[str]):
    ...
    for entry in auth_app.get_all_entries():
        ...
        if guardian_data.is_guarded:
            ...
            if guardian_data.active_guardian == entry.get_guardian():
                ...
                print(
                    f"... active guardian [red]is not same[/red] as the one in the auth registration file (bad flow, please handle separately)"
                )

            print("... please see: https://docs.dharitri.org/developers/built-in-functions/#setguardian")
        ...
        if guardian_data.pending_guardian:
            ...
            if guardian_data.pending_guardian == entry.get_guardian():
                ...
                print(
                    f"... pending guardian [red]is not same[/red] as the one in the auth registration file (bad flow, please handle separately)"
                )
    ...
    ux.confirm_continuation(
        f"Ready to set guardians, by sending [green]{len(transactions_wrappers)}[/green] transactions?"
    )
