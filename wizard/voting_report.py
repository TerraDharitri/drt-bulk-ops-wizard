...
def _do_main(cli_args: list[str]):
    ...
    for contract in configuration.liquid_staking_contracts:
        ...
        governance_records_for_liquid_staking_contracts[contract] = GovernanceRecord.load_many_from_proofs_file(
            proofs_path
        )
    ...
    for account_wrapper in accounts_wrappers:
        ...
        if previous_direct_vote:
            print(
                "\t",
                f"previous direct vote on {format_time(previous_direct_vote.timestamp)}:",
                previous_direct_vote.vote_type,
            )
        if previous_vote_via_legacy_delegation:
            print(
                "\t",
                f"previous delegated vote (legacy delegation) on {format_time(previous_vote_via_legacy_delegation.timestamp)}:",
                previous_vote_via_legacy_delegation.vote_type,
            )
        ...
        for contract in configuration.liquid_staking_contracts:
            ...
            if previous_vote:
                print(
                    "\t",
                    f"previous delegated vote ({contract}) on {format_time(previous_vote.timestamp)}:",
